<template>
	<view v-if="isShow" class="picker picker-show-mask">
		<!-- 日期选择器 -->
		<view v-if="type!='time'" class="picker-modal">
			...
			<view v-if="type == 'timerange'" class="picker-quickly-selectors">
				<view v-for="(item, index) in quicklyItems" class="picker-quickly-selector" :key="index" @click="onQuicklySelectDate(item)">
					{{item.name}}
				</view>
			</view>
...
<script>
	...
	const DateTools = {
		...
		/**
		 * 获取日期到以指定时间作为基准的相关偏移的年年头(不改变原来的date对象)
		 * @param d Date对象
		 * @param o 偏移量
		 * @return Date对象
		 */
		getYearStartDate(d, o) {
			let n = new Date(d);
			n.setFullYear(n.getFullYear() + o, 0, 1);	//定向到偏移年的1月1，注意月份这块是从0开始的，即[0]=>1
			return n;
		},
		/**
		 * 获取日期到以指定时间作为基准的相关偏移的年年末(不改变原来的date对象)
		 * @param d Date对象
		 * @param o 偏移量
		 * @return Date对象
		 */
		getYearEndDate(d, o) {
			let n = new Date(d);
			n.setFullYear(n.getFullYear() + o, 11, 31);	//定向到偏移年的12月31，注意月份这块是从0开始的，即[0]=>1
			return n;
		},
		/**
		 * 获取日期到以指定时间作为基准的相关偏移的月份月头(不改变原来的date对象)
		 * @param d Date对象
		 * @param o 偏移量
		 * @return Date对象
		 */
		getMonthStartDate(d, o) {
			let n = new Date(d);
			n.setMonth(n.getMonth() + o, 1);
			return n;
		},
		/**
		 * 获取日期到以指定时间作为基准的相关偏移的月份月末(不改变原来的date对象)
		 * @param d Date对象
		 * @param o 偏移量
		 * @return Date对象
		 */
		getMonthEndDate(d, o) {
			let n = new Date(d);
			n.setMonth(n.getMonth() + 1 + o, 0);	//和上面是多了一个月，置到下个月月初的前一天
			return n;
		},
		/**
		 * 获取日期以指定时间作为基准的相关偏移的周的周日(不改变原来的date对象)→注意当前开始周是从周日开始
		 * @param d Date对象
		 * @param o 指定的周偏移量
		 * @return Date对象
		 */
		getWeekStartDate(d, o) {
			let n = new Date(d);
			let c = n.getDay(); //当前周索引，用当前日期减去周索引可得到周起始日期
			n.setDate(d.getDate() - c + o * 7);	//设置为偏移量o下的的一周起始
			return n;
		},
		/**
		 * 获取日期到指定的周的周六(不改变原来的date对象)→当前开始周的结束日期是周六
		 * @param d Date对象
		 * @param o 指定的月份
		 * @return Date对象
		 */
		getWeekEndDate(d, o) {
			let n = new Date(d);
			let c = n.getDay(); //当前周索引
			let e = 7 - c - 1;	//距离当前周最后一天（周六）还有几天
			n.setDate(d.getDate() + e + o * 7);	//设置为偏移量o下的的一周末尾
			return n;
		},
		/**
		 * 获取日期以指定时间作为基准的相关偏移的周的周日(不改变原来的date对象)→注意当前开始周是从周已开始
		 * @param d Date对象
		 * @param o 指定的周偏移量
		 * @return Date对象
		 */
		getISOWeekStartDate(d, o) {
			let n = new Date(d);
			let c = n.getDay(); //当前周索引
			c -= 1;	//iso格式的周格式是从周一开始的，和正常的Date对象的getDay比较要大1
			n.setDate(d.getDate() - c + o * 7);	//设置为偏移量o下的的一周起始
			return n;
		},
		/**
	...
	export default {
		...
		data() {
			return {
				...
				// quicklyItems<Array>：快速选择项
				// name：显示文本
				// type：快速选择对应类型，参考值: week，month，year
				// offset：默认0，代表本周期。其他值代表增减
				quicklyItems: [{
					name: '本周',
					type: 'isoWeek',
					offset: 0
		...
		methods: {
			...
			// 快速选择时间
			onQuicklySelectDate(item) {
				let beginTime, endTime;
				// 接受item参数。item对应quicklyItems中的值
				if(!item) {
					return console.error('需要有有效的快速选择项，请填写quicklyItems变量')
				}
				if(!item.type || item.type === '') {
					return console.error('需要有有效的快速选择项类型，请填写quicklyItems变量中的type。有效值请参考示例值')
				}
				if(!item.offset) item.offset = 0;
				
				let date = new Date();	//快速选择时，仅将当前时间作为判断基准点
				switch (item.type) {
					case 'week':
						beginTime = DateTools.getWeekStartDate(date, item.offset);
						endTime = DateTools.getWeekEndDate(date, item.offset);
						break;
					case 'isoWeek':
						beginTime = DateTools.getISOWeekStartDate(date, item.offset);
						endTime = DateTools.getISOWeekEndDate(date, item.offset);
						break;
					case 'month':
						beginTime = DateTools.getMonthStartDate(date, item.offset);
						endTime = DateTools.getMonthEndDate(date, item.offset);
						break;
					case 'year':
						beginTime = DateTools.getYearStartDate(date, item.offset);
						endTime = DateTools.getYearEndDate(date, item.offset);
						break;
					default:
						return console.error('快速选择类型有误，请检查')
						break;
				}
				...
				this.checkeds = [];
				this.checkeds.push(...[beginTime, endTime]);
				this.onConfirm();
			},
			//取消
			onCancel() {
...
<style lang="scss" scoped>
	...
	.picker {
		...
		&-time {
			...
			left: calc((750rpx - $calendar-size) / 2 + 40rpx) !important;
		}
		...
		&-modal {
			...
			position: absolute;
			top: 50%;
			left: calc((750rpx - $calendar-size) / 2);
			width: $calendar-size;
