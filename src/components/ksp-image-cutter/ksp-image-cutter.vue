<template>
<view v-if="url && url !== ''" class="ksp-image-cutter">
	<canvas class="target-canvas" :style="{width: target.width + 'px', height: target.height + 'px'}" canvas-id="target"></canvas>
	<view class="body">
		<image v-if="url && url !== ''" class="image" @load="imageLoad" :style="{left: image.left + 'px', top: image.top + 'px', width: image.width + 'px', height: image.height + 'px'}" :src="url"></image>
