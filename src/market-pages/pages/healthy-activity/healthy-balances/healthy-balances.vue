  ...
  <view class="flex-page">
    ...
    <view class="time-filter-bar">
      <view class="time-filter-trigger" @tap="chooseRange">
        ...
        <view class="time-filter-val" :class="{'time-filter-placeholder': timeFilterTexts.length != 2}">
          {{ timeFilterTexts.length == 2 ? (timeFilterTexts[0] + ' ~ ' + timeFilterTexts[1]) : '开始时间~结束时间' }}
        </view>
    ...
    <view class="flex-page-content">
      ...
      <view class="list-box">
        <scroll-view scroll-y class="tab-page-scroll" @scrolltolower="getList">
            ...
            <view v-for="(item, index) in dataList" class="income" :key="index">
              <view class="income-content">
                <view class="income-info">
                  <view class="income-desc" :class="{'warning-color': item.change_type == 3}">
                    {{ typeText[item.change_type] }} {{ item.order_user_name ? ('-来自' + item.order_user_name) : '' }}
                  </view>
          ...
          <view v-if="dataList.length > 0 && currentPage > pageTotal && !listLoading" class="scroll-tips">已经到底啦
          </view>
          <view v-if="dataList.length == 0 && currentPage > pageTotal && !listLoading" class="scroll-tips">暂无记录哦
          </view>
...
export default {
  ...
  methods: {
    getList() {
      ...
      this.$get('/healthy/balance', params, res => {
