<template>
  <view class="flex-page">
      ...
      <view class="order-infos">
        <view class="order-info">
          ...
          <view class="pply-auto-trigger-box">
            <switch class="pply-auto-trigger" :disabled="userInfo.c_vip_level != 1" :checked="preOrder.autoPpyl" @change="toggleAutoPP" @tap="validAutoPp"/>
          </view>
        </view>
    ...
    <view class="order-ctrls">
      ...
      <view v-if="preOrder.realPayPrice == '0.00'" class="order-ctrl-btn" :class="{'disabled': !couldPay}" @tap="tryReStart">立即重开</view>
      <view v-else class="order-ctrl-btn" :class="{'disabled': !couldPay}" @tap="tryReStart">立即重开</view>
    </view>
...
export default {
  ...
  methods: {
    getPreOrder(handler) {
      ...
      if(this.preOrder.pay_type == 4) {
        params.pay_no = this.preOrder.pay_no;
        params.pay_order_sn = this.preOrder.pay_order_sn;
      }
    ...
    // 自动参团
    toggleAutoPP(e) {
      this.preOrder.autoPpyl = e.detail.value;
      this.getPreOrder();
    },
    validAutoPp() {
