      ...
      <view class="withdraw-content">
        ...
        <view class="withdraw-content-box">
          ...
          <view class="withdraw-selects">
            <view v-for="(item, index) in quiklySelectOptions" class="withdraw-select"
              :class="{'active': quicklySelectIndex == index}" :key="index" @tap="quicklyUpdatePrice(index)">
              {{item.title}}
            </view>
          </view>
        </view>
        ...
        <view class="withdraw-content-box withdraw-price">
          ...
          <view class="withdraw-price-box">
            ...
            <input class="withdraw-price-input" type="digit" placeholder="请填写" :value="formData.total_price"
              @input="updatePrice" />
          </view>
      ...
      <view class="bankcode-popup-wrapper">
        <view class="bankcode-popup-content">
          ...
          <view class="bankcode-input-box">
            ...
            <input class="bankcode-input" type="number" placeholder="请填写本人银行卡号" :value="editing_bank_account"
              @input="updateEditingAccount" />
          </view>
...
export default {
  ...
  methods: {
    ...
    getSignType(){
      let params = {
        from:'conf',
        type: 9 //1基本信息，2大礼包入口图地址，3使用须知，4用户协议，5隐私政策， 6银行卡快捷支付用户协议 不传为全部返回
      }
      this.$get('/system/info',params,(res)=>{
        this.withdrawPayType=res.data.withdrawPayType
      })
    },
    getSignData() {
      let params = {
      ...
      this.$get('/kuaishang/ContractInfo', params, res => {
        this.signData = res.data.returnData;
        this.signData.related_user = res.data.name;
        ...
        uni.showModal({
          title: '提示',
          content: `${this.$written.h5ShareTitle}平台已经为您缴纳所有税费 ，请您前往填写个人信息和签约合同哦`,
          showCancel: false,
          success: res => {
          }
        })
    ...
    widthdraw() {
      if(!this.allowPay) {
        uni.showModal({
          ...
          success: res => {
            if(res.confirm) {
              if(this.withdrawPayType==5){
                  uni.navigateTo({
                  url: '/market-pages/pages/agreement-pay/china-digital-Internet-technology/china-digital-Internet-technology'
                })
            }else if(this.withdrawPayType==4){
            let companyId = config.kuaishangOptions.companyId;
            // #ifndef H5
            uni.navigateToMiniProgram({
              appId: 'wx01e9e17c8c07189c',
              path: 'pages/webview/webview?url=' + encodeURIComponent(`https://sign.linghuoxin.com/faindex?companyId=${companyId}&env=miniprogram&thirdId=${this.userInfo.uid}`),
            });
            // #endif
            // #ifdef H5
            window.location.href = `https://sign.linghuoxin.com/faindex?companyId=${companyId}&thirdId=${this.userInfo.uid}`
            // #endif
        }
            }
          }
      ...
      let params = {
        real_name: this.signData.username,
        total_price: this.formData.total_price,
        type: this.formData.type,
        bank_account: this.formData.bank_account,
        withdraw_type: 7, //回收集品券
        uid: this.userInfo.uid
      };
      ...
      //   });
      // }
      else if(parseFloat(params.total_price) > this.userInfo.crowd_balance) {
        return uni.showModal({
          title: '提示',
    ...
    confirmBankcode() {
      ...
      // }
      let params={
          bank_account:this.editing_bank_account,
          id:this.signData.id,
          uid:uni.getStorageSync('userInfo').uid
      };
      this.$post('/zsk/ContractEdit',params,(res)=>{
        this.getSignData()
      })
