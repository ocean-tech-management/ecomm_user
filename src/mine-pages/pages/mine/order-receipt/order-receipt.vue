...
export default {
  ...
  methods: {
    ...
    getPoster() {
      let params = {
        order_sn: this.receipt.order_sn
      }
      this.$sget('/order/receipt', params, res => {
        uni.showLoading({title: '生成专属图中'});
        if(!res.data.images || res.data.images === '') return this.tryLocalDraw();
        this.getPosterPath(res.data.images);
      }, err => {
        this.tryLocalDraw();
      })
    },
    ...
    drawPoster() {
      ...
      return new Promise( (resolve, reject) => {
        ...
        ctx.fillText('下单回执', 30,70);  //画标题
        ctx.restore();

        fontSize = 24;
        ctx.font = `normal ${fontSize}px/1.4 "pingfangSC"`;
        ctx.fillText(`订单单号：${that.receipt.order_sn}`, 30,133);  //画订单单号
        ctx.restore();

        fontSize = 24;
        ctx.setFontSize(fontSize);
        ctx.fillText(`下单时间：${that.receipt.pay_time}`, 30,172); //画订单时间
        ctx.restore();

        // 收货人信息
        fontSize = 30;
        ctx.font = `500 ${fontSize}px/1.4 "pingfangSC"`;
        ctx.fillText('收货人：', 50,350); //画收货人
        ...
        ctx.fillText(`${shipName}　${shipPhone}`, 50 + offsetLeft + 15,352); //画收货人信息
        ctx.restore();

        fontSize = 30;
        ctx.font = `500 ${fontSize}px/1.4 "pingfangSC"`;
        ctx.fillText('地　址：', 50,417); //画收货地址
        ...
        ctx.fillText(`${shipAddress}`, 50 + offsetLeft + 15,419); //画收货地址信息
        ctx.restore();

        // 画一条边框
        ctx.setFillStyle('#eeeeee');  //设置边框色
        ctx.fillRect(0, 489, 710, 2);
        ctx.restore();
