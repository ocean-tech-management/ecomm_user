      ...
      <scroll-view scroll-y class="tab-page-scroll" :refresher-enabled="true" :refresher-triggered="refreshing"
        ...
        <view class="page-entrances">
          ...
          <view class="page-entrance" @tap="undo">
            ...
            <view class="page-entrance-title">集品券专区</view>
          </view>
        </view>
        <view class="activity-entrances">
          <view v-for="(item, index) in specialPayList" :key="index" class="activity-entrance" @tap="goToActivity(item)">
            <image :src="item.icon" class="activity-entrance-image"></image>
          </view>
        </view>
        <view v-if="roundList && roundList.length > 0" class="page-area-tabs-box">
          <view class="page-area-tabs">
            <view v-for="(item, index) in roundList" class="page-area-tab" :class="{ 'active': roundIndex == index }"
              :key="index" @tap="updateRoundIndex(index)">
              {{ item.title }}
            </view>
          </view>
        ...
        <view v-if="timeList && timeList.length > 0" class="page-time-tabs-box">
          ...
          <scroll-view class="page-time-tabs" :scroll-into-view="activeTimeTab" enable-flex scroll-x>
            <view v-for="(item, index) in timeList" :id="item.timeId" class="page-time-tab"
              :class="{ 'active': item.status == 2 }" :key="index">
...
export default {
  ...
  methods: {
    // 获取特殊支付方式活动列表(美丽金、美丽豆、礼品券)
    getSpecialPayList() {
      this.$sget('/home/specialPayList', null, res => {
        const data = res.data
        if (!data || data.length === 0) return
        this.specialPayList = data
      })
    },
    getData() {
    ...
    getRounds(handing) {
      this.$get('/crowdFunding/activityList', null, res => {
      ...
      if(this.dataList && this.dataList.length > 0) {
        for(let i = 0; i < this.dataList.length; i++) {
          ...
          if(
            ...
            this.$set(this.dataList[i], 'validResult', 'valid');
          } else if(
            moment(this.dataList[i].start_time).diff(date, 'seconds') > 0
          ) {
            // this.dataList[i].validResult = 'unstart';
            this.$set(this.dataList[i], 'validResult', 'unstart');
          } else {
            // this.dataList[i].validResult = 'novalid';
            this.$set(this.dataList[i], 'validResult', 'novalid');
          }
        }
      }
    ...
    goToActivity(data) {
      const { id, title } = data
      let url = '/market-pages/pages/active/actives/actives'
      url += `?id=${id}&title=${title}&limit_type=1`
      // const url = `/market-pages/pages/active/actives/actives?id=120&limit_type=1`;
      this.validTourist(() => {
        uni.navigateTo({
          url: url
        })
